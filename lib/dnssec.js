...
dnssec.sign = function sign(sig, priv, rrset) {
  ...
  s.labels = countLabel(rrset[0].name);
...
dnssec.verifyDS = function verifyDS(keyMap, pds) {
  for (const rr of pds) {
    const parent = rr.data;
    const dnskey = keyMap.get(parent.keyTag);

    if (!dnskey)
      continue;

    const ds = dnssec.createDS(dnskey, parent.digestType);
  ...
  return false; // Missing KSK
};

dnssec.verifyRRSIG = function verifyRRSIG(msg, keyMap) {
  for (const section of [msg.answer, msg.authority]) {
    if (section.length === 0)
      continue;
