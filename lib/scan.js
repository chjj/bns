...
function readUnknown(wire, iter, hdr, origin, file) {
  ...
  if (tok1.string !== '\\#')
    throw new ParseError('bad unknown serialization', tok1, file);
  ...
  try {
    ...
    throw new ParseError('bad size serialization', tok2, file);
  }

  const [hex, comment] = concat(iter, file);

  if (size !== (hex.length >>> 1))
    throw new ParseError('invalid hex size', tok2, file);

  let data;
  try {
    data = util.parseHex(hex);
  } catch (e) {
    throw new ParseError('invalid unknown hex', tok2, file);
  }
...
function readType(rd, type, iter, origin, file) {
  ...
  switch (type) {
    ...
    case schema.SERVERS: {
      const names = [];
      const [items, comment] = collect(iter, file);
      ...
      for (const item of items) {
        ...
        try {
          ...
          throw new ParseError('bad rendezvous server', null, file);
        }
    ...
    case schema.HEXEND: {
      const [str, comment] = concat(iter, file);
      ...
      try {
        ...
        throw new ParseError('invalid base64 string', null, file);
      }
    ...
    case schema.BASE32: {
      ...
      try {
        ...
        throw new ParseError('invalid base32 string', null, file);
      }
    ...
    case schema.WKS: {
      // Supposed to be ONE rdata element (in parens).
      const tok = expect(iter, tokens.STRING, file);
      const map = schema.parseWKS(tok.string);
      return [map, skip(iter, file)];
    }
