...
class DNSServer extends EventEmitter {
  ...
  send(res, rinfo, req) {
    ...
    const max = req ? req.maxSize() : res.maxSize();
    const raw = res.compress(!tcp, max);
  ...
  async handle(msg, rinfo) {
    ...
    res = new Response(this, rinfo, req);
    res.respond(req);
    res.ra = this.ra;
...
class Response extends Message {
  ...
  send() {
    this.server.send(this, this.rinfo, this._req);
  }
}
