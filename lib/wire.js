...
const options = {
  LLQ: 0x1, // Long Lived Queries
  UL: 0x2, // Update Lease Draft
  NSID: 0x3, // NSID
  DAU: 0x5, // DNSSEC Algorithm Understood
  DHU: 0x6, // DS Hash Understood
  N3U: 0x7, // NSEC3 Hash Understood
  SUBNET: 0x8, // Client Subnet
  EXPIRE: 0x9, // EDNS0 expire
  COOKIE: 0xa, // EDNS0 Cookie
  TCPKEEPALIVE: 0xb, // EDNS0 TCP Keep-Alive
  PADDING: 0xc, // EDNS0 Padding
  LOCALSTART: 0xfde9, // Beginning of range reserved for local/experimental use
  LOCALEND: 0xfffe // End of range reserved for local/experimental use
};
...
class Message {
  ...
  respond(req) {
    ...
    this.response = true;
    this.recursionDesired = req.recursionDesired;
    this.checkingDisabled = req.checkingDisabled;
  ...
  isEDNS0() {
    return this.getEDNS0() !== null;
  }

  setEDNS0(usize = 4096, dok = false) {
    const rr = new Record();
    rr.name = '.';
    rr.type = types.OPT;
    rr.version = 0;
