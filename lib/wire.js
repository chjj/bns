...
class Message extends bio.Struct {
  ...
  collect(name, type) {
    ...
    for (const rr of this.answer) {
      ...
      switch (rr.type) {
        ...
        case types.SMIMEA:
        case types.SRV:
          rrname = util.from(rr.name, 2);
          break;
        case types.NSEC3:
          rrname = util.from(rr.name, 1);
          break;
      }

      if (!util.equal(rrname, target))
        continue;
  ...
  bodyLength() {
    let body = 0;
    body += this.answer.length;
    body += this.authority.length;
    body += this.additional.length;
    if (this.edns.enabled)
      body += 1;
    return body;
  }
  ...
  write(bw, items) {
    ...
    bw.writeU16BE(this.additional.length + (this.edns.enabled ? 1 : 0));
  ...
  toString(ms, host, port) {
    ...
    let additional = this.additional.length;
    ...
    str += ` ADDITIONAL: ${additional}\n`;

    if (this.edns.enabled) {
      str += '\n';
      str += this.edns.toString();
    }
    ...
    let sig = null;
    let arcount = this.additional.length;

    if (arcount > 0) {
      const rr = this.additional[arcount - 1];
      if (rr.type === types.SIG && rr.data.typeCovered === 0) {
        arcount -= 1;
        sig = rr;
      }
    }
  ...
  fromString(str) {
    ...
    if (arcount > 0) {
      ...
      while (arcount--) {
        ...
        this.additional.push(Record.fromString(line));
      }
    }
...
class EDNS extends bio.Struct {
  ...
  fromJSON(json) {
    ...
    this.size = json.size;
    this.code = stringToEcode(json.code);
...
class Question extends bio.Struct {
  ...
  toString() {
    ...
    const class_ = shortToString(this.class);
  ...
  fromString(str) {
    ...
    this.name = parts[0];
    this.class = stringToShort(parts[1]);
  ...
  getJSON() {
    return {
      name: this.name,
      class: shortToString(this.class),
  ...
  fromJSON(json) {
    ...
    this.name = json.name;
    this.class = stringToShort(json.class);
...
class OPTRecord extends RecordData {
  ...
  fromString(str) {
    ...
    while (i < parts.length) {
      ...
      const body = parts.slice(i, i + s.length + 1).join(' ');
...
class Option extends bio.Struct {
  ...
  fromJSON(json) {
    ...
    this.code = stringToOption(json.code);
...
class OptionData extends bio.Struct {
  ...
  schema() {
    ...
    if (!s)
      return schema.options[options.UNKNOWN];
