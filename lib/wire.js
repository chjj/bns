...
class Message extends bio.Struct {
  ...
  getSizes(udp, map) {
    ...
    if (this.edns.enabled) {
      const sz = this.edns.getSize(map);
      if (udp && size + sz > max)
        return [osize, oitems];
      size += sz;
      items += 1;
    ...
    if (this.tsig) {
      const sz = this.tsig.getSize(map);
      if (udp && size + sz > max)
        return [osize, oitems];
      size += sz;
      items += 1;
    ...
    if (this.sig0) {
      const sz = this.sig0.getSize(map);
      if (udp && size + sz > max)
        return [osize, oitems];
      size += sz;
      items += 1;
...
class EDNS extends bio.Struct {
  ...
  write(bw, map) {
    ...
    bw.writeU16BE(this.getDataSize(map, false));
...
class RecordData extends bio.Struct {
  ...
  toString() {
    const schema = lazy(require, './schema');
    return schema.toString(exports, this, this.schema());
  }
  ...
  getJSON() {
    const schema = lazy(require, './schema');
    return schema.toJSON(exports, this, this.schema());
  }

  fromJSON(json) {
    const schema = lazy(require, './schema');
    return schema.fromJSON(exports, this, this.schema(), json);
  }
}
