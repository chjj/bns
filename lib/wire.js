...
class Message extends Struct {
  ...
  *records() {
  ...
  compress(udp, max) {
    if (!MSG_BUFFER)
      MSG_BUFFER = Buffer.allocUnsafe(MAX_MSG_SIZE);

    const bw = new StaticWriter(MSG_BUFFER);
    const map = new Map();
