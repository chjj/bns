...
class Message extends bio.Struct {
  ...
  *records() {
  ...
  getSizes(udp, map) {
    ...
    if (this.edns.enabled) {
      const val = this.edns.getSize(map);
      if (udp && size + val > max)
        return [osize, oitems];
      size += val;
      items += 1;
    ...
    if (this.tsig) {
      const val = this.tsig.getSize(map);
      if (udp && size + val > max)
        return [osize, oitems];
      size += val;
      items += 1;
    ...
    if (this.sig0) {
      const val = this.sig0.getSize(map);
      if (udp && size + val > max)
        return [osize, oitems];
      size += val;
      items += 1;
...
class EDNS extends bio.Struct {
  ...
  write(bw) {
    ...
    bw.writeU16BE(this.getDataSize());
...
class HIPRecord extends RecordData {
  ...
  canonical() {
    ...
    for (let i = 0; i < this.servers.length; i++) {
      const name = this.servers[i];
      this.servers[i] = name.toLowerCase();
    }
