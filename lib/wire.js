...
class Record extends bio.Struct {
  ...
  fromString(str) {
    const parts = util.parts(str, 0, 7);

    let name = '.';
    let ttl = 0;
    let class_ = classes.IN;
    let i = 0;

    // Try to find name.
    if (i < parts.length) {
      const part = util.part(str, parts, i);
      if (encoding.isName(part)) {
        name = part;
        i += 1;
      }
    }
    ...
    // Find the type.
    const typeName = util.part(str, parts, i);
    i += 1;

    // Convert to type and RD.
    const type = stringToType(typeName);
    ...
    if (i + 1 < parts.length) {
      const part = util.part(str, parts, i);
      if (util.startsWith(typeName, 'TYPE') && part === '\\#') {
        unknown = true;
        i += 1;
      }
    }

    // If unknown, parse the hex.
    let data;
    if (unknown) {
      const part = util.part(str, parts, i);
      i += 1;

      const len = util.parseU32(part);

      let body = '';

      if (i < parts.length) {
        const [off] = parts[i];
        body = str.substring(off);
        body = util.stripSP(body);
        if (body === '-')
          body = '';
      }

      const rd = util.parseHex(body);
      ...
      if (!RD)
        throw new Error(`Unknown record type: ${typeName}.`);
...
class Option extends bio.Struct {
  ...
  fromString(str) {
    const parts = util.parts(str, 0, 4);

    // Find the code name.
    const codeName = util.part(str, parts, 0);
    const code = stringToOption(codeName);
    const OD = optsByVal[code];

    // Test to see if this option
    // was serialized as unknown.
    let unknown = false;
    if (parts.length >= 3) {
      const part = util.part(str, parts, 1);
      if (util.startsWith(codeName, 'OPTION') && part === '\\#')
        unknown = true;
    }

    // If it was unknown, parse the hex.
    let option;
    if (unknown) {
      const part = util.part(str, parts, 2);
      const len = util.parseU32(part);
      ...
      const data = util.parseHex(body);

      assert(data.length === len);

      if (OD)
        option = OD.decode(data);
      else
        option = UNKNOWNOption.decode(data);
    } else {
      if (!Option)
        throw new Error(`Unknown option code: ${codeName}.`);
