 ...
 *
 * Parts of this software are based on golang/go and miekg/dns:
...
// EDNS0 Option codes.
const options = {
  LLQ: 0x1, // long lived queries: http://tools.ietf.org/html/draft-sekar-dns-llq-01
  UL: 0x2, // update lease draft: http://files.dns-sd.org/draft-sekar-dns-ul.txt
  NSID: 0x3, // nsid (See RFC 5001)
  DAU: 0x5, // DNSSEC Algorithm Understood
  DHU: 0x6, // DS Hash Understood
  N3U: 0x7, // NSEC3 Hash Understood
  SUBNET: 0x8, // client-subnet (See RFC 7871)
  EXPIRE: 0x9, // EDNS0 expire
  COOKIE: 0xa, // EDNS0 Cookie
  TCPKEEPALIVE: 0xb, // EDNS0 tcp keep alive (See RFC 7828)
  PADDING: 0xc, // EDNS0 padding (See RFC 7830)
  LOCALSTART: 0xfde9, // Beginning of range reserved for local/experimental use (See RFC 6891)
  LOCALEND: 0xfffe, // End of range reserved for local/experimental use (See RFC 6891)
  _DO: 1 << 15 // DNSSEC OK
};
...
class Message {
  ...
  inject(msg) {
    ...
    this.truncated = msg.truncated;
    this.recursionDesired = msg.recursionDesired;
    this.recursionAvailable = msg.recursionAvailable;
    this.zero = msg.zero;
    this.authenticatedData = msg.authenticatedData;
    this.checkingDisabled = msg.checkingDisabled;
    this.code = msg.code;
...
class Record {
  ...
  get ok() {
    return (this.ttl & options._DO) !== 0;
  }

  set ok(v) {
    if (!v)
      this.ttl &= ~options._DO;
    else
      this.ttl |= options._DO;
  }
}
...
class ARecord extends RecordData {
  ...
  toWriter(bw) {
    bw.copy(IP.toBuffer(this.address), 12, 16);
    return bw;
  }
...
class AAAARecord extends RecordData {
  ...
  toWriter(bw) {
    bw.writeBytes(IP.toBuffer(this.address));
    return bw;
  }
  ...
  toWriter(bw) {
    ...
    switch (this.gatewayType) {
      ...
      case 1:
        ...
        bw.copy(IP.toBuffer(this.ip4), 12, 16);
        break;
      case 2:
        // assert(this.ip6.length === 16);
        bw.writeBytes(IP.toBuffer(this.ip6));
        break;
      case 3:
  ...
  fromReader(br) {
    ...
    switch (this.gatewayType) {
      case 0:
        break;
      case 1:
        this.ip4 = IP.toString(br.readBytes(4));
        break;
      case 2:
        this.ip6 = IP.toString(br.readBytes(16));
        break;
      case 3:
...
class L32Record extends RecordData {
  ...
  toWriter(bw) {
    bw.writeU16BE(this.preference);
    bw.copy(IP.toBuffer(this.locator32), 12, 16);
    return bw;
  }
...
function decodeOption(code, data) {
  switch (code) {
    ...
    case options.LOCALSTART:
    case options.LOCALEND:
