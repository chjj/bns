...
class Ownership {
  ...
  _getClaim(proof) {
    ...
    for (const rr of zone.claim) {
      ...
      try {
        result = this.parseClaim(rr.name, rd.txt);
      } catch (e) {
        continue;
      }
  ...
  verifySignatures(proof, anchors) {
    ...
    for (; i < proof.zones.length - 1; i++) {
      ...
      ds = extractSet(zone.referral, '', types.DS);
    }
  ...
  checkSanity(zone, parent, isLast) {
    ...
    const zoneLabels = countLabels(zoneName);
    ...
    if (zone.referral.length > 0) {
      ...
      const dsLabels = countLabels(dsName);
  ...
  verifyRecords(rrs, zskMap) {
    ...
    for (const rr of rrs) {
      ...
      const rrset = extractSet(rrs, rr.name, s.typeCovered);
  ...
  isChild(parent, child) {
    ...
    const labels = countLabels(parent);

    if (countLabels(child) !== labels + 1)
      return false;
  ...
  async prove(name) {
    ...
    try {
      return await this._prove(stub, name);
    } finally {
      await stub.close();
    }
  }

  async _prove(stub, name) {
  ...
  async _lookup(stub, name, type) {
    ...
    if (result.length === 0
        || !util.hasType(result, type)
        || !util.hasType(result, types.RRSIG)) {
    ...
    for (const rr of answer) {
      ...
      switch (rr.type) {
        case types.DS: {
          ...
          const e = ds.get(rd.keyTag);

          // Prefer higher security and de-duplicate.
          if (!e || e.digestType < rd.digestType)
            ds.set(rd.keyTag, rr);
