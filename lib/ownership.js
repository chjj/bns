...
class Ownership {
  ...
  parseData(proof, name, items, extra) {
    return {
      name,
      items
    };
  ...
  addData(proof, items) {
    ...
    assert(this.claim.length > 0);

    const rr = new Record();
    const rd = new TXTRecord();

    rr.name = this.claim[0].name;
    rr.type = types.TXT;
    rr.class = this.claim[0].class;
    rr.ttl = this.claim[0].ttl;
    rr.data = rd;
  ...
  filterClaim(proof, rrs) {
    ...
    for (const rr of rrs) {
      ...
      if (rr.type === types.TXT) {
        if (this.shouldIgnore(proof, rr.name, rr.data.txt))
          continue;
      }
  ...
  _getData(proof, extra) {
    ...
    for (const rr of zone.claim) {
      ...
      try {
        result = this.parseData(proof, rr.name, rd.txt, extra);
      } catch (e) {
        if (e.code === 'ERR_ASSERTION')
          throw e;
        continue;
      ...
      if (rr.type === types.RRSIG) {
        ...
        if (sig) {
          ...
          if (type === types.DNSKEY) {
            ...
            if (!kskSet.has(sd.keyTag) && kskSet.has(rd.keyTag)) {
              sig = rr;
              continue;
            }
