...
class HSMUser {
  ...
  sign(dnskey, rrset) {
    ...
    let hmsg = hash.digest(data);

    const mechanism = algToMechanism[alg];
    if (mechanism === pkcs11js.CKM_RSA_PKCS) {
      ...
      hmsg = Buffer.concat([prefix, hmsg]);
    }
    ...
    rd.signature = this.pkcs11.C_Sign(this.session, hmsg, out);
