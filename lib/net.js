...
class Base extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.socket.on('message', (msg, rinfo) => {
      ...
      const mapped = IP.isMapped(ip);
      this.emit('message', msg, {
        tcp: false,
        family: mapped ? 'IPv4' : 'IPv6',
...
class Client extends Base {
  ...
  async write(msg, port, host) {
    const key = IP.toHostname(host, port);
...
class Server extends Base {
  ...
  constructor(options) {
    ...
    this.server.on('connection', (conn) => {
      ...
      const key = IP.toHostname(socket.host, socket.port);
      this.sockets.set(key, socket);
    });
  }
...
class TCPSocket extends EventEmitter {
  ...
  init() {
    this.parser.on('message', (data) => {
      const rinfo = {
        tcp: true,
        family: IP.isV4String(this.host) ? 'IPv4' : 'IPv6',
        address: this.host,
        port: this.port
      };
  ...
  cleanup() {
    ...
    const key = IP.toHostname(this.host, this.port);
