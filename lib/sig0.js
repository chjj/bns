...
sig0.createKey = function createKey(algorithm, publicKey, private) {
  ...
  if (algorithm === sig0.algs.PRIVATEDNS) {
    if (!private)
      throw new Error('Private algorithm is required.');

    assert(privateAlgs.hasOwnProperty(private));
    const privateAlg = privateAlgsbyVal[private];

    assert(privateAlg.length <= MAX_NAME_SIZE);

    const key = bio.write(1 + privateAlg.length + 1 + publicKey.length);
    key.writeU8(privateAlg.length);
    key.writeNullString(privateAlg);
...
sig0.privateAlgs = privateAlgs;
sig0.privateAlgsbyVal = privateAlgsbyVal;
sig0.hashes = hashes;
