...
sig0.sign = function sign(msg, key, priv, signer) {
  ...
  let arcount = msg.readUInt32BE(10, true);
  ...
  if (pos !== -1) {
    ...
    arcount -= 1;
    buf.writeUInt32BE(arcount, 10, true);
    msg = buf;
  }
  ...
  out.writeUInt32BE(arcount + 1, 10, true);
...
sig0.verify = function verify(msg, key, verifier) {
  ...
  const arcount = msg.readUInt32BE(10, true);
  ...
  data.writeUInt32BE(arcount - 1, rdlen + 10, true);
