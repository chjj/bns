...
sig0.createKey = function createKey(algorithm, publicKey) {
  ...
  // PRIVATEDNS publicKey should be prefixed with algo name and null byte 
  if (algorithm === sig0.algs.PRIVATEDNS) {
    const name = "secp256k1sha256";
    const key = bio.write(1 + name.length + 1 + publicKey.length);
    key.writeU8(name.length);
    key.writeNullString(name);
