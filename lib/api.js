...
class API {
  constructor(createResolver = defaultResolver) {
    assert(typeof createResolver === 'function');

    // Private
    this._conf = conf.clone();
    this._createResolver = createResolver;

    // Public
    this.Resolver = function Resolver() {
      return new API(createResolver);
    };
    ...
    this.FAILURE = 'EFAILURE'; // custom
    this.INSECURE = 'EINSECURE'; // custom
  }

  async _resolver() {
    return this._createResolver(this._conf.clone(), hosts);
  }
  ...
  async _query(name, type) {
    ...
    const resolver = await this._resolver();

    try {
      await resolver.open();
    } catch (e) {
      throw makeQueryError(name, type, this.FAILURE, e);
    }
    ...
    try {
      ...
      throw makeQueryError(name, type, this.BADRESP, e);
    }
...
function makeError(name, syscall, code, e) {
  const orig = e ? `: ${e.message}` : '';
  const err = new Error(`${syscall} ${code} ${name}${orig}`);
  err.errno = code;
  err.code = code;
  err.syscall = syscall;
  err.hostname = name;
...
function makeQueryError(name, type, code, e) {
  const syscall = `query${constants.typeToString(type)}`;
  return makeError(name, syscall, code, e);
}

function makeGAIError(name, code, e) {
  return makeError(name, 'getaddrinfo', code, e);
}

function makeGNIError(name, code, e) {
  return makeError(name, 'getnameinfo', code, e);
}
