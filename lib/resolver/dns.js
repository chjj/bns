...
class DNSResolver extends EventEmitter {
  ...
  retry(query, rotate, forceTCP, isTimeout) {
    ...
    // Fix strange edge case with afilias.
    // Issue appeared the evening of 2018-09-28.
    //
    // To reproduce:
    //   $ dig @199.19.57.1 org. DNSKEY +dnssec +nord
    // To fix:
    //   $ dig @199.19.57.1 org. DNSKEY +dnssec +nord +bufsize=1280
    //
    // The second `dig` will cause the response
    // to be truncated, re-requesting it over TCP.
    if (isTimeout && req.edns.enabled && req.edns.size > STD_EDNS_SIZE) {
      req = req.clone();
