...
keys.createPublic = function createPublic(algorithm, raw) {
  ...
  switch (algorithm) {
    ...
    case algs.RSASHA256:
    case algs.RSASHA512: {
      return crypto.convertRSA(raw);
    }
    case algs.ECDSAP256SHA256: {
      return crypto.convertP256(raw);
    }
    case algs.ECDSAP384SHA384: {
      return crypto.convertP384(raw);
    }
    case algs.ED25519: {
      return crypto.convertED25519(raw);
    }
    case algs.ED448: {
      return crypto.convertED448(raw);
    }
    default: {
      throw new Error(`Unsupported algorithm: ${algToString(algorithm)}.`);
    }
