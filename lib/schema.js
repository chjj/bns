...
function toString(wire, rd, schema) {
  ...
  for (let i = 0; i < schema.length; i++) {
    ...
    const value = rd[name];
    str.push(writeType(wire, type, value, rd));
  }

  return str.join(' ');
}

function writeType(wire, type, value, rd) {
  assert((type >>> 0) === type);
...
function fromJSON(wire, rd, schema, json) {
  ...
  for (let i = 0; i < schema.length; i++) {
    const [name, type] = schema[i];
    rd[name] = readJSON(wire, type, json[name], rd);
  }
...
function toJSON(wire, rd, schema) {
  ...
  for (let i = 0; i < schema.length; i++) {
    const [name, type] = schema[i];
    json[name] = writeJSON(wire, type, rd[name], rd);
  }

  return json;
}

function readJSON(wire, type, value, rd) {
  assert((type >>> 0) === type);
...
function writeJSON(wire, type, value, rd) {
  assert((type >>> 0) === type);
