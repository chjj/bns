...
util.isName = function isName(s) {
  ...
  try {
    sizeName(util.fqdn(s), null, false, false);
    return true;
  } catch (e) {
    return false;
  }
...
util.splitLF = function splitLF(str) {
  assert(typeof str === 'string');
  return str.trim().split(/\n+/);
};

util.splitSP = function splitSP(str) {
  assert(typeof str === 'string');
  return str.trim().split(/[ \t\v]+/);
};
...
util.splitLines = function splitLines(str, escaped) {
  assert(typeof str === 'string');
...
util.parts = function parts(str, off, limit) {
  assert(typeof str === 'string');
  assert((off >>> 0) === off);
...
util.part = function part(str, chunks, i) {
  assert(typeof str === 'string');
