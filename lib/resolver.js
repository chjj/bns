...
const ROOT_KEYS = [
  {
    ...
    algorithm: 8,
    publicKey: ''
      + 'AwEAAYvgWbYkpeGgdPKaKTJU3Us4YSTRgy7+dzvfArIhi2tKoZ/WR1Df'
      + 'w883SOU6Uw7tpVRkLarN0oIMK/xbOBD1DcXnyfElBwKsz4sVVWmfyr/x'
      + '+igD/UjrcJ5zEBUrUmVtHyjar7ccaVc1/3ntkhZjI1hcungAlOhPhHlk'
      + 'MeX+5Azx6GdX//An5OgrdyH3o/JmOPMDX1mt806JI/hf0EwAp1pBwo5e'
      + '8SrSuR1tD3sgNjr6IzCdrKSgqi92z49zcdis3EaY199WFW60DCS7ydu+'
...
class RecursiveResolver extends DNSResolver {
  ...
  async checkSignatures(m, auth, pds) {
    const [qs, hit, res, keyMap] = await this.lookupDNSKEY(auth);

    if (pds.length > 0) {
      if (!dnssec.verifyDS(keyMap, pds))
        throw new Error('Invalid DS.');
    }

    if (!dnssec.verifyRRSIG(m, keyMap))
      throw new Error('Invalid RRSIG.');

    if (!hit)
      this.cache.insert(qs, auth.zone, res, false);
  }
  ...
  async resolve(question, auth, pds) {
    ...
    for (let i = 0; i < MAX_REFERRALS; i++) {
      ...
      if (!isRoot && pds.length > 0) {
        await this.checkSignatures(res, auth, pds);
        ad = true;
      }
  ...
  async lookup(name, type, auth, pds) {
    const qs = new Question(name, type);
    return this.resolve(qs, auth, pds);
  }

  async reverse(addr, auth, pds) {
    const name = encoding.reverse(addr);
    return this.lookup(name, types.PTR, auth, pds);
  }
}
...
function hashQuestion(qs, zone) {
  // If qs.name=mail.google.com. && zone=.
  // Then name=com.
  // If qs.name=mail.google.com. && zone=com.
  // Then name=google.com.
  // If qs.name=mail.google.com. && zone=google.com.
  // Then name=mail.google.com.
  const name = currentName(qs.name, zone);
  if (name.length < qs.name.length)
    return name;
  return `${name}${qs.type}`;
}
