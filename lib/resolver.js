...
class RecursiveResolver extends DNSResolver {
  ...
  async findNS(rc, name, type) {
    ...
    const [hops, res] = await this._resolve(qs, ns, rc.hops);
    ...
    const auth = new Authority(name, addr.data.address);
  ...
  async resolve(qs, ns) {
    ...
    const [hops, res] = await this._resolve(qs, ns, 0);

    this.log('Finishing resolving %s/%d (hops=%d).', qs.name, qs.type, hops);
...
class ResolveContext {
  ...
  hop() {
    if (this.hops >= MAX_REFERRALS)
      throw new Error('Maximum referalls exceeded.');
