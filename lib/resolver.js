...
class RecursiveResolver extends DNSResolver {
  ...
  async resolve(question, ns, ds) {
    ...
    let auth = ns;
    let chain = ds.length > 0;
    let answer = null;
    let parent = '~';
    let zones = [ns.zone];

    this.log('Querying %s/%d.', qs.name, qs.type);
    ...
    for (let i = 0; i < MAX_REFERRALS; i++) {
      ...
      if (res.answer.length > 0) {
        const [ok, cname, rrs] = isAlias(res.answer, qs);
        ...
        if (ok) {
          ...
          if (chain) {
            chain = ds.length > 0;
            if (!chain)
              this.log('Trust chain broken due to alias.');
          }
    ...
    if (answer.answer.length > 0)
      this.cache.insert(question, ns.zone, answer, chain);

    return extractAnswer(answer, question, chain, chased);
  }

  async lookup(name, type, ns, ds) {
    const qs = new Question(name, type);
    return this.resolve(qs, ns, ds);
  }

  async reverse(addr, ns, ds) {
    const name = encoding.reverse(addr);
...
function extractAnswer(m, qs, ad, chased) {
  const res = m.clone();
  ...
  switch (alias.type) {
    case types.CNAME: {
      if (qs.type === types.CNAME || qs.name !== alias.name)
        return [false, '', null];
      return [true, alias.data.target, filtered];
    }
    case types.DNAME: {
