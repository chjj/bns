...
class RecursiveResolver extends DNSResolver {
  ...
  async findNS(name, type, prc) {
    const qs = new Question(name, type);
    const ns = this.randomAuthority();
    const rc = new ResolveContext(qs, ns);
    const res = await this.iterate(rc, prc);
  ...
  async lookupNS(name, prc) {
    if (prc.hops > MAX_REFERRALS)
    ...
    if (this.inet6) {
      ...
      try {
        return await this.findNS(name, types.AAAA, prc);
      } catch (e) {
    ...
    return this.findNS(name, types.A, prc);
  }
  ...
  async pickAuthority(authority, additional, prc) {
    ...
    if (zones.size === 0) {
      ...
      const auth = await this.lookupNS(ns, prc);
      auth.zone = zone;
  ...
  async handleAuthority(rc, prc) {
    ...
    const auth = await this.pickAuthority(authority, additional, prc);
    ...
    if (rc.chain) {
      ...
      if (rc.ds.length === 0) {
        ...
        const [r, hit] = await this.ask(qs, rc.auth);
  ...
  async next(rc, prc) {
    ...
    if (rc.res.authority.length > 0
        && (rc.res.code === codes.NOERROR
        || rc.res.code === codes.YXDOMAIN)) {
      return this.handleAuthority(rc, prc);
    }
  ...
  async iterate(rc, prc) {
    this.log('Querying %s/%d.', rc.qs.name, rc.qs.type);

    while (prc.hops < MAX_REFERRALS) {
      if (!await this.next(rc, prc))
        break;
      prc.hops += 1;
    }
...
class Cache {
  constructor() {
    this.map = new Map();
    this.queue = new Heap((a, b) => a.deadline - b.deadline);
    this.size = 0;
  }
