...
class RecursiveResolver extends DNSResolver {
  ...
  async ask(qs, auth) {
    const [h, r] = await this.hook(qs, auth);
  ...
  async checkSignatures(msg, auth, ds) {
    ...
    const [hit, res, keyMap] = await this.lookupDNSKEY(qs, auth);
  ...
  async handleAuthority(rc) {
    ...
    if (!negative) {
      for (const rr of rc.res.authority) {
    ...
    if (rc.res.aa && !negative && rc.chain) {
      if (nsec.length === 0)
  ...
  async next(rc) {
    ...
    // BIND calls the server "insane" at this point.
    return false;
  }
