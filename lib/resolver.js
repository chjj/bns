...
class DNSResolver extends EventEmitter {
  ...
  handle(msg, rinfo) {
    ...
    if (res.code !== codes.NOERROR) {
      this.pending.delete(id);
      item.reject(new Error('Unexpected rcode.'));
      return;
    }
...
class RecursiveResolver extends DNSResolver {
  ...
  async _resolve_(question, ns, root) {
  ...
  insert(rc) {
    if (!rc.hit)
      this.cache.insert(rc.qs, rc.auth.zone, rc.res, rc.chain);
  }

  async handleDS(rc) {
    this.log('Verifying zone change: [%s->%s]', rc.parent.zone, rc.auth.zone);
  ...
  async handleAnswer(rc) {
    ...
    rc.aliases.add(alias);

    this.insert(rc);
    rc.parent = rc.auth;
    rc.auth = this.randomAuthority();
    rc.zones.push(rc.auth.zone);
    rc.ds = [];
    rc.refs += 1;

    this.log('Found alias to: %s', alias);
    this.log('Alias changed zone: [%s->%s]', rc.parent.zone, rc.auth.zone);
  ...
  async handleAuthority(rc) {
    ...
    rc.parent = rc.auth;
    rc.auth = auth;
    rc.zones.push(auth.zone);
    rc.refs += 1;

    this.log('Switching authority: [%s] (%s)', auth.host, auth.name);
    this.log('Switching zone: [%s->%s]', rc.parent.zone, auth.zone);
  ...
  async next(rc) {
    await this.askNext(rc);

    if (this.chain)
      await this.handleDS(rc);
...
function isLame(req, res) {
  ...
  for (const rr of res.authority) {
    if (rr.type !== types.NS)
      continue;
    if (util.isSubdomain(name, rr.name))
      return true;
  }
