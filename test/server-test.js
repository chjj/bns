...
describe('Server', function() {
  ...
  describe('Abstract DNS Server', function() {
    let server, dns;
    ...
    it('should point resolver at server', async () => {
      dns.setServers(['127.0.0.1:5300']);
      const res = dns.getServers();
      assert.deepStrictEqual(res, ['127.0.0.1:5300']);
    });
  ...
  describe('Extended DNS Server', function () {
    const ports = {
      a: 5301,
      r: 5302
    };
    ...
    after(async () => {
      ...
      assert.strictEqual(authQueries, 21);
      assert.strictEqual(recQueries, 1);
    });

    describe('Authoritative', function () {
      it('should open authoritative server', async () => {
        await authServer.bind(ports.a, '127.0.0.1');
      });
      ...
      it('should do a recursive resolution', async () => {
        ...
        res.setStub('127.0.0.1', ports.a, Record.fromString(KSK_2010));
    ...
    describe('Recursive', function () {
      it('should open recursive server', async () => {
        await recServer.bind(ports.r, '127.0.0.1');
      });
